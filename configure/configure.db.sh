...
LOCATION="https://download.postgresql.org/pub/repos/yum/10/redhat/rhel-7-x86_64/pgdg-centos10-10-1.noarch.rpm"

sudo yum install -y $LOCATION
sudo yum install -y postgresql10-server-10.1  # there is a compatibility issue with 10.2
...
echo "shared_preload_libraries = 'timescaledb'"  | sudo tee -a $DATA_FOLDER/postgresql.conf
